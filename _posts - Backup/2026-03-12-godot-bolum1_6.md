...
Şimdi `BlinkingLight`'ın rengini değiştir ve kaydet — şehirdeki tüm `BrokenLantern`'lar anında güncellenir. Tek tek müdahale etmeye gerek yok.
...
Godot'da herhangi bir sahneden **kalıtım** da alabilirsiniz. Bir Godot sahnesi şunlardan herhangi biri olabilir:
...
Bu yapı, saf kodda bir **sınıf (class)** gibi davranır — ama sahnenizi editörü kullanarak, sadece kodla ya da ikisini harmanlayarak tasarlama özgürlüğüne sahipsiniz.

Örneğin `Character` adında bir temel karakter sahnesi oluşturup, ondan kalıtım alarak `Magician` (büyücü) sahnesi yapabilirsiniz. `Character` sahnesinde bir değişiklik yaptığınızda, `Magician` da otomatik olarak güncellenir.
...
Dışarıdan araç kullanmak isterseniz Godot'nun import eklentisi sistemi buna da izin verir. GDScript dilinin var olmasının da bir sebebi bu: Oyun geliştiricilerinin ve oyun tasarımcılarının ihtiyaçlarına göre tasarlanmış, motorla ve editörle sıkı sıkıya entegre bir dil.
...
> **Not:** GDExtension ile C, C++, Rust, D, Haxe veya Swift gibi derlenen dillerle yüksek performanslı kod yazabilirsiniz — motoru yeniden derlemeye gerek yok.

Bir uyarı da ekleyeyim: **3D çalışma alanı**, 2D'ye kıyasla daha az yerleşik araca sahip. Karmaşık karakter animasyonları veya arazi düzenleme gibi işler için harici programlara ya da eklentilere ihtiyaç duyabilirsiniz.
...
Açık kaynak olmanın pratik bir avantajı daha var: **Hata ayıklama kolaylığı.** Godot, hatalar için stack trace (hata izi) ile birlikte mesaj yazdırır — hata motorun kendi içinden bile gelse. Kodun ne yaptığını her zaman takip edebilirsiniz.
...
Birkaç tam zamanlı çekirdek geliştirici olsa da, projenin binlerce katkıcısı bulunmaktadır. Gönüllü programcılar kendilerinin de ihtiyaç duyduğu özellikleri geliştirdiğinden, her büyük sürümde motorun farklı köşelerinde iyileştirmeler görürsünüz.
...
Editör, motorun kendi UI (Kullanıcı Arabirimi) sistemini kullanır. Projeyi test ederken kod ve sahneleri **hot-reload** edebilir, hatta oyun kodunu editör içinde doğrudan çalıştırabilir.

Bu şu anlama gelir: Oyunlarınız için kullandığınız kodlar ve sahneler, editörü genişletmek için de kullanılabilir. **`@tool`** ek açıklamasını herhangi bir GDScript dosyasının başına eklerseniz, o kod editör içinde çalışır.
...
- Projelerinizde kullandığınız node'lar ve API ile editör scriptleri yazmak
...
> **Teknik not:** Editörün kendisi C++ ile yazılmıştır ve statik olarak derlenir. Bu yüzden onu `project.godot` dosyası olan normal bir proje gibi içe aktaramazsınız.

---

## Ayrı 2D ve 3D Motorları

Godot, ayrı **2D ve 3D render motorları** sunar. 2D sahnelerinin temel birimi **pikseldir**. Motorlar ayrı olsa da şunları yapabilirsiniz:
...
| **Editör = Godot oyunu** | Editörü genişletmek için aynı araçları kullanırsınız |
...
Godot'nun neden böyle tasarlandığını artık biliyorsunuz. Bu temel anlayış, ilerleyen bölümlerde node sistemi, sahne yapısı ve GDScript ile çalışırken pek çok "neden böyle?" sorusuna zaten cevap verecek.
